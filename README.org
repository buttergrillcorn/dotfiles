...
- *Mod + Tab*: Cycle focus to next window
...
- Conditional swayidle timeouts based on power state:
  - Battery: 5min screen off, 10min lock, 30min suspend-then-hibernate
  - Plugged in: 10min screen off, 15min lock, 1hr suspend-then-hibernate
- Lid close: screen off, lock, and suspend-then-hibernate
...
Denote integrates with ox-hugo for publishing notes as blog posts. The Hugo site lives at =~/denote/blog/= and follows Hugo's standard directory structure.

***** Features
- *ID-based tracking*: Safely handles note renames without breaking exports
- *Diff checking*: Shows changes before overwriting existing posts
- *Denote templates*: Auto-inserts ox-hugo properties when creating blog notes
- *Hugo-compliant export*: Markdown files go to =~/denote/blog/content/blog/=
- *No conflicts*: Original org files remain untouched, denote always opens org files
- *Selective publishing*: Only notes with =blog= keyword are published
- *Batch publishing*: Publish all blog posts at once or individually
...
├── blog/                    # Hugo site root
│   ├── content/
│   │   └── blog/           # Published blog posts (.md files)
│   ├── themes/
│   ├── static/
│   ├── config.toml
│   └── ...
└── *.org                   # Your denote notes (source files)
#+end_example

***** Creating Blog Posts
Create a note with the =blog= keyword using standard denote commands:
1. Use =SPC d n= or =SPC d d= to create a new note
2. Add =blog= as one of the keywords when prompted
3. Denote template automatically adds ox-hugo properties:
   - =#+hugo_base_dir: ~/denote/blog/= - Hugo site root
   - =#+hugo_section: blog= - Exports to =content/blog/=
   - =#+hugo_tags:= - Populated from denote keywords (excluding 'blog')
   - =#+hugo_draft: false= - Publishes immediately

***** Publishing to Markdown
- =SPC d p= - Publish current note (prompts to add =blog= keyword if missing)
- =SPC d P= - Publish all notes with =blog= keyword

When publishing:
1. Checks if note has =blog= keyword (prompts to add if missing)
2. Ensures ox-hugo properties exist (adds smart defaults if missing)
3. Exports to markdown with ID-based filename tracking
4. Shows diff if updating existing post (asks for confirmation)
5. Handles renames safely (finds old export by ID, shows diff, deletes old file)

Exported markdown files are saved to =~/denote/blog/content/blog/= following Hugo's standard content structure.

***** Workflow Example
#+begin_example
1. SPC d n → Create new note with title "My First Post" and keyword "blog"
2. Write your content in the org file (saved in ~/denote/)
3. SPC d p → Publish to markdown
4. Markdown file created: ~/denote/blog/content/blog/20251107T123456--my-first-post__blog.md
5. Original org file: ~/denote/20251107T123456--my-first-post__blog.org (untouched)
6. Later, rename the note with SPC d r r
7. SPC d p → Shows diff and safely updates (old file deleted, new file created)
8. Build Hugo site: cd ~/denote/blog && hugo
#+end_example

***** Configuration
You can customize the Hugo paths by editing these variables in =dotfiles.org=:
#+begin_src elisp
