...
- *Hugo-compliant export*: Markdown files go to =~/denote/blog/content/posts/=
...
├── blog/                    # Hugo site root
│   ├── content/
│   │   └── posts/          # Exported blog posts (.md files)
│   ├── themes/
│   ├── static/
│   ├── config.toml
│   └── ...
...
2. Creates denote note with =blog= keyword in =~/denote/=
3. Automatically adds ox-hugo properties:
   - =#+hugo_base_dir: ~/denote/blog/= - Hugo site root
   - =#+hugo_section: posts= - Exports to =content/posts/=
...
Exported markdown files are saved to =~/denote/blog/content/posts/= following Hugo's standard content structure.
...
4. Markdown file created: ~/denote/blog/content/posts/my-first-post.md
5. Original org file: ~/denote/20251107T123456--my-first-post__blog.org (untouched)
6. Build Hugo site: cd ~/denote/blog && hugo
#+end_example
