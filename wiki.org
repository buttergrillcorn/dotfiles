...
(defun wiki/rename-note ()
  "Rename the current wiki note and update its title and filename.
Updates the #+TITLE property, renames the file (slugified), and updates
the org-id database to maintain link validity."
  (interactive)
  (unless (and (buffer-file-name) (wiki//in-wiki-buffer-p))
    (user-error "Not in a wiki note"))
  (let* ((current-title (or (cadr (assoc "TITLE" (org-collect-keywords '("TITLE"))))
                            (file-name-sans-extension (file-name-nondirectory (buffer-file-name)))))
         (new-title (read-string "New note title: " current-title))
         (new-slug (wiki//slugify new-title))
         (new-filename (concat new-slug ".org"))
         (new-path (expand-file-name new-filename org-directory))
         (old-path (buffer-file-name)))
    (when (string-equal new-title current-title)
      (user-error "Title unchanged"))
    (when (and (not (string-equal (file-name-nondirectory old-path) new-filename))
               (file-exists-p new-path))
      (user-error "File %s already exists" new-filename))
    ...
    ;; Update org-id database with new location
    (org-id-update-id-locations (list new-path))
    
    ;; Save to trigger front-matter updates (EXPORT_FILE_NAME)
    (save-buffer)
...
(defun wiki/insert-link ()
  ...
  (let* ((default-directory org-directory)
    ...
    (when file
      ...
      (let* ((original-title (if (string-suffix-p ".org" file)
...
Handles duplicate headings by including outline path."
  (let ((file-path (expand-file-name file org-directory)))
        ...
        (save-excursion
          ...
          (while (re-search-forward org-outline-regexp nil t)
            (let* ((heading (org-get-heading t t t t))
                   ...
                   ;; Format: "Parent / Child / Heading" to ensure uniqueness and context
                   (candidate (if path
                                  (format "%s / %s" (string-join path " / ") heading)
                                heading)))
              ;; Handle exact duplicates (same path + heading) by appending spaces
              (while (assoc candidate candidates)
                (setq candidate (concat candidate " ")))
              (push (cons candidate line) candidates))))
        (setq candidates (nreverse candidates))
        (if (not candidates)
            (progn (message "No headings found in %s" file) nil)
          (consult--read candidates
                         :prompt (format "Heading (%s): " (file-name-nondirectory file))
                         ...
                         :state (lambda (action cand)
                                  (when (and (eq action 'preview) cand)
                                    (let ((line (cdr (assoc cand candidates))))
                                      (when line
                                        (consult--preview-at-point
                                         (consult--position-marker (find-file-noselect file-path) line 0))))))))))))

(defun wiki/insert-heading-link ()
  "Insert an org-id link to a heading in a wiki file.
If text is selected, use it as the link description and replace it.
Uses markers to safely track selection even if buffer is modified (e.g. ID creation)."
  (interactive)
  (let* ((default-directory org-directory)
         ;; Use markers to track positions robustly across buffer modifications
         (start-marker (when (use-region-p) (copy-marker (region-beginning))))
         (end-marker   (when (use-region-p) (copy-marker (region-end))))
         (description  (when (and start-marker end-marker)
         ...
         (selected-file (consult--read
                         ...
                         :require-match t)))
    (when selected-file
      (let ((line-number (wiki//prompt-for-heading selected-file)))
        (when line-number
          (let* ((full-path (expand-file-name selected-file org-directory))
                 (id-title
                  (with-current-buffer (find-file-noselect full-path)
                    (save-excursion
                      (goto-char (point-min))
                      ...
                      (cons (wiki//ensure-heading-id)
                            (org-get-heading t t t t))))))
            ;; Delete the original selection if it existed
...
(defun org-hugo-link--org-id-to-path (link)
    ...
    (condition-case err
        (let* ((id (org-element-property :path link))
               ...
               ;; Capture source file to detect same-page links
            ...
            (with-current-buffer (marker-buffer marker)
              (save-excursion
                ...
                (let* ((buffer-file-name (buffer-file-name))
                       ...
                       ;; Use org-get-heading to strip TODOs, priorities, and tags cleanly
                       (heading-text (org-get-heading t t t t))
                       ;; Use org-hugo-slug if available to match Hugo's anchor generation exactly
                       (slug (when heading-text
                               (if (fboundp 'org-hugo-slug)
                                   (org-hugo-slug heading-text)
                                 (wiki//slugify heading-text))))
                  ...
                  ;; If linking to the same file, use relative hash to ensure smooth scrolling
                  (if (and slug 
                           source-file 
                           buffer-file-name
                           (string= (expand-file-name source-file) 
                                    (expand-file-name buffer-file-name)))
                      (setq path (concat "#" slug))
...
(map! :leader
      (:prefix ("n" . "notes")
       ...
       :desc "Rename note" "r" #'wiki/rename-note
